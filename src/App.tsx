...
export default function App() {
  ...
  const [lastAlertMark, setLastAlertMark] = useState(0); // 알림 버킷 체크용
  ...
  const calculateStatsLocally = (allRuns: Run[]) => {
    ...
    const weekly = Array.from({ length: 4 }).map((_, i) => {
      ...
      const e = endOfWeek(d);
      const weekRuns = allRuns.filter(r => new Date(r.timestamp) >= s && new Date(r.timestamp) <= e);
      return {
        name: `${format(s, 'MM/dd')}`,
  ...
  const loadAllData = () => {
    const saved = localStorage.getItem('stride_v4_runs');
  ...
  const deleteRun = (id: number) => {
    ...
    setRuns(updated);
    localStorage.setItem('stride_v4_runs', JSON.stringify(updated));
  ...
  const saveRun = () => {
    ...
    setRuns(updatedRuns);
    localStorage.setItem('stride_v4_runs', JSON.stringify(updatedRuns));
  ...
  const startTracking = () => {
    startSilentAudio();
    setDistance(0); setDuration(0); setLastAlertMark(0);
      ...
      watchId.current = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          if (lastPosition.current) {
            ...
            // 2m 이상 이동 & 오차범위 50m 이내 데이터만 수용 (연결성 강화)
            if (d > 2 && pos.coords.accuracy < 50) {
        ...
        { 
          ...
          timeout: 20000 // GPS 초기 연결 대기 시간 연장
        }
      );
    }
  };
  ...
  const calculatePaceFormatted = (distMeters: number, timeSeconds: number) => {
    if (distMeters < 10) return "0.00분/1km";
    const totalMinutes = timeSeconds / 60;
    const totalKm = distMeters / 1000;
    const paceDecimal = totalMinutes / totalKm;
  ...
  // --- [FIX] 정확한 시간 기반 음성 알림 로직 ---
  useEffect(() => {
    if (isTracking && audioEnabled && duration > 0) {
      // 현재 몇 번째 알림 구간인지 계산
      const currentBucket = Math.floor(duration / alertInterval);
      
      if (currentBucket > lastAlertMark) {
        const mins = currentBucket * (alertInterval / 60);
        const distKm = (distance / 1000).toFixed(2);
        
        const utterance = new SpeechSynthesisUtterance(
          `${mins}분 경과했습니다. 현재 이동 거리는 ${distKm} 킬로미터입니다.`
        );
        window.speechSynthesis.speak(utterance);
        
        setLastAlertMark(currentBucket); // 알림 완료 상태 업데이트
      }
    }
  }, [duration, isTracking, audioEnabled, alertInterval, lastAlertMark, distance]);
    ...
    return (
      <div className="p-6 space-y-8 pb-24 h-full overflow-y-auto">
        <div className="flex justify-between items-center">
          <div><h1 className="text-4xl font-black text-slate-900 tracking-tight">StrideTrack</h1><p className="text-slate-500 font-medium italic text-sm">GPS Record System</p></div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between h-32">
            <div className="flex items-center gap-2 mb-2 text-emerald-500"><Zap size={16} /><span className="text-xs font-bold uppercase tracking-widest">Last Pace</span></div>
            <p className="text-2xl font-black text-slate-900">{latestPace}<span className="text-xs font-normal text-slate-400 ml-1">분/km</span></p>
          </div>
        </div>
  ...
  const renderActiveRun = () => (
    <div className="h-full flex flex-col bg-slate-900 text-white p-8">
      ...
      <div className="pb-10">
        <button onClick={() => { saveRun(); stopTracking(); setIsTracking(false); setView('dashboard'); }} className="w-full bg-rose-500 text-white font-black py-5 rounded-3xl text-xl shadow-2xl active:scale-95 transition-transform">FINISH RUN</button>
      </div>
    </div>
  );
    ...
    return (
      <div className="p-6 space-y-6 pb-24 h-full overflow-y-auto">
        ...
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <h3 className="text-sm font-black text-slate-900 flex items-center gap-2 mb-6"><BarChart3 size={18} className="text-emerald-500" /> Distance (km)</h3>
    ...
    <div className="max-w-md mx-auto h-screen bg-slate-50 overflow-hidden font-sans relative">
      ...
      {view !== 'active-run' && (
        <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-xl border-t border-slate-100 px-8 py-4 flex justify-between items-center z-50">
          <button onClick={() => setView('dashboard')} className={cn("p-2 flex flex-col items-center gap-1 transition-all", view === 'dashboard' ? "text-emerald-600 scale-110" : "text-slate-300")}><Play size={20} fill={view === 'dashboard' ? "currentColor" : "none"} /><span className="text-[10px] font-black uppercase tracking-widest">Run</span></button>
